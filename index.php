...
$result = mysqli_query($conn, "SELECT * FROM available_doc_nurse_dept WHERE Dep_name = '$dept'");
if (!$result) {
    printf("Error: %s\n", mysqli_error($conn));
    exit();
}
...
<h2>MySQL: Select Avialable Doctor and Nurse table data</h2>
<table border='1'>
			
	<tr>
		<th>Doctor on call doctor name</th>
		<th>Doctor position</th>
		<th>Nurse on call name</th>
    ...
    ?>        
            <tr>
                ...
                <td><?php echo $row['Dpositon']; ?></td>
				<td><?php echo $row['Nname']; ?></td>
				<td><?php echo $row['Npositon']; ?></td>
